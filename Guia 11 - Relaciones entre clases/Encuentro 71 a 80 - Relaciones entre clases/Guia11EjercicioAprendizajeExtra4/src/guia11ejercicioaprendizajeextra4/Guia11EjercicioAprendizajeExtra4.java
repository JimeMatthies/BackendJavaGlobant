...
@author JimeM
 */
package guia11ejercicioaprendizajeextra4;
...
public class Guia11EjercicioAprendizajeExtra4 {

    public static void main(String[] args) {
        Simulator sim = new Simulator();
        int amount = 100;
        List<Student> students = sim.generateStudents(amount, sim.generateNames(amount), sim.generateSSN(amount));
        sim.showStudents(students);
        List<Vote> votes = sim.voting(students, 3);
        sim.showVotes(votes);
        sim.voteCount(students, votes);
        sim.showFacilitadores(students, 5);
    }
